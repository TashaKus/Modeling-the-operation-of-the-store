...
def cashBoxNumber_handler(widget):
...
def build(app):
	...
	'''= createBox('Количество касс :', 
		box_style = Pack(direction=ROW), 
		label_style = Pack(text_align=RIGHT, width = 210))'''
	cashBoxNumber_text = toga.TextInput(on_change = cashBoxNumber_handler)
...
def main():
    return toga.App('Shop', 'org.beeware.helloworld', startup=build)
